    ...
    if (true_position > 1) {
        ...
        if (true_position != hk_timestamp_array_size) { // check self if won't cause underflow
            ...
            if (timestamp > timestamps[true_position] &&
                timestamp - timestamps[true_position] <= threshold) { // edge case left is max index. bigger value
                                                                      // than at max
                return true_position;
            } else if (timestamps[true_position] - timestamp <= threshold) { // edge case left is max index.
                                                                             // smaller value than at max
                return true_position;
            }
        }
...
Result dynamic_timestamp_array_handler(uint16_t num_items) {
    ...
    if (num_items != hk_timestamp_array_size) {
        ...
        if (timestamps != NULL && hk_timestamp_array_size < num_items) { // check if growing because we delete
                                                                         // everything if shrinking
...
Result store_config(uint8_t rewrite_all) {
    ...
    int32_t fout = red_open(hk_config,
...
Result write_hk_to_file(uint16_t filenumber, All_systems_housekeeping *all_hk_data) {
    int32_t fout = red_open(fileName,
                            RED_O_CREAT | RED_O_RDWR); // open or create file to write binary
    if (fout == -1) {
