...
uint16_t MAX_FILES = 20160;  // value is 20160 (7 days) based on 30 second
                             // period
char fileName[] = "VOL0:/tempHKdata.TMP";
uint16_t current_file = 1;  // Increments after file write. loops back at
                            // MAX_FILES 1 indexed
char hk_config[] = "VOL0:/HKconfig.TMP";
static uint8_t config_loaded = 0;  // set to 1 after config is loaded

uint32_t *timestamps =
    0;  // This is a dynamic array to handle file search by timestamp
uint16_t hk_timestamp_array_size =
    0;  // NOT BYTES. stored as number of items. 1 indexed. 0 element unused
...
uint16_t get_file_id_from_timestamp(uint32_t timestamp) {
  uint32_t threshold = 15;  // How many seconds timestamps need to be within.
                            // Currently assumes 30 second hk intervals
  ...
  if (true_position > 1) {
    if (timestamp - timestamps[true_position - 1] <=
        threshold) {  // check lower neighbour if exists
      return true_position - 1;
    }
    if (true_position !=
        hk_timestamp_array_size) {  // check self if won't cause underflow
      ...
      if (timestamp > timestamps[true_position] &&
          timestamp - timestamps[true_position] <=
              threshold) {  // edge case left is max index. bigger value than at
                            // max
        return true_position;
      } else if (timestamps[true_position] - timestamp <=
                 threshold) {  // edge case left is max index. smaller value
                               // than at max
        return true_position;
      }
    }
  } else {  // left must be min index
    if (timestamps[true_position] - timestamp <=
        threshold) {  // edge case left is min index. smaller than min
      return true_position;
    }
  }
  return 0;
...
Result dynamic_timestamp_array_handler(uint16_t num_items) {
  ...
  if (num_items != hk_timestamp_array_size) {
    uint32_t size = sizeof(*timestamps) *
                    (num_items + 1);  //+1 to allow non-zero index room
    uint32_t *tmp = (uint32_t *)pvPortMalloc(size);
    if (tmp == NULL) {
      return FAILURE;
    }
    if (timestamps != NULL &&
        hk_timestamp_array_size <
            num_items) {  // check if growing because we delete everything if
                          // shrinking
    ...
    uint16_t i;
    for (i = (hk_timestamp_array_size + 1); i <= num_items;
         i++) {  // ensure new entries are clean
      timestamps[i] = 0;
    }
...
/*populate struct by calling appropriate functions*/
  ...
  ADCS_returnState ADCS_return_code =
      HAL_ADCS_getHK(&all_hk_data->adcs_hk);  // ADCS get housekeeeing
#endif                                        // ADCS_IS_STUBBED

#ifndef ATHENA_IS_STUBBED
  int Athena_return_code =
      Athena_getHK(&all_hk_data->Athena_hk);  // Athena get temperature
  ...
  // Adding these in order to get the startup telemetry - should be added
  // elsewhere
  eps_refresh_startup_telemetry();
  eps_startup_telemetry_t eps_startup = get_eps_startup_telemetry();
  EPS_getHK(&all_hk_data->EPS_hk,
            &all_hk_data->EPS_startup_hk);  // EPS get housekeeping
  ...
  if (xSemaphoreTake(uTransceiver_semaphore, 0) == pdTRUE) {
    ...
    UHF_return UHF_return_code =
        UHF_getHK(&all_hk_data->UHF_hk);  // UHF get housekeeping
  }
#endif  // UHF_IS_STUBBED

#ifndef SBAND_IS_STUBBED
  STX_return STX_return_code =
      HAL_S_getHK(&all_hk_data->S_band_hk);  // S_band get housekeeping
...
static Found_file exists(const char *filename) {
  int32_t file;
  red_errno = 0;
  file = red_open(filename, RED_O_CREAT | RED_O_EXCL |
                                RED_O_RDWR);  // attempt to create file
...
Result store_config(uint8_t rewrite_all) {
  ...
  int32_t fout = red_open(
      hk_config,
      RED_O_CREAT | RED_O_RDWR);  // open or create file to write binary
  if (fout == -1) {
  ...
  if (exist == 0 || rewrite_all == 1) {
    red_write(fout, timestamps,
              ((hk_timestamp_array_size + 1) * sizeof(uint32_t)));
  } else {
 ...
 *      FAILURE or SUCCESS
 */
Result write_hk_to_file(uint16_t filenumber,
                        All_systems_housekeeping *all_hk_data) {
  int32_t fout = red_open(
      fileName,
      RED_O_CREAT | RED_O_RDWR);  // open or create file to write binary
  if (fout == -1) {
    ex2_log("Unexpected error %d from red_open()\r\n", (int)red_errno);
    ex2_log("Failed to open or create file to write: '%s'\n", fileName);
    return FAILURE;
  }
  ...
  red_write(fout, &all_hk_data->hk_timeorder,
 ...
 *      FAILURE or SUCCESS
 */
Result read_hk_from_file(uint16_t filenumber,
                         All_systems_housekeeping *all_hk_data) {
...
/**
 * @brief
 *      Public. Performs all calls and operations to retrieve hk data and store
 * it
 * @return
 ...
 *      enum for SUCCESS or FAILURE
 */
Result load_historic_hk_data(uint16_t file_num,
                             All_systems_housekeeping *all_hk_data) {
 ...
 *      enum for success or failure
 */
Result fetch_historic_hk_and_transmit(csp_conn_t *conn, uint16_t limit,
                                      uint16_t before_id,
  ...
  while (limit > 0) {
    ...
    memcpy(&packet->data[OUT_DATA_BYTE + used_size], &all_hk_data.adcs_hk,
           sizeof(all_hk_data.adcs_hk));
    used_size += sizeof(all_hk_data.adcs_hk);
    memcpy(&packet->data[OUT_DATA_BYTE + used_size], &all_hk_data.Athena_hk,
           sizeof(all_hk_data.Athena_hk));
    used_size += sizeof(all_hk_data.Athena_hk);
    memcpy(&packet->data[OUT_DATA_BYTE + used_size], &all_hk_data.EPS_hk,
           sizeof(all_hk_data.EPS_hk));
    used_size += sizeof(all_hk_data.EPS_hk);
    memcpy(&packet->data[OUT_DATA_BYTE + used_size], &all_hk_data.UHF_hk,
           sizeof(all_hk_data.UHF_hk));
    used_size += sizeof(all_hk_data.UHF_hk);
    memcpy(&packet->data[OUT_DATA_BYTE + used_size], &all_hk_data.S_band_hk,
           sizeof(all_hk_data.S_band_hk));
...
SAT_returnState hk_service_app(csp_conn_t *conn, csp_packet_t *packet) {
  ...
  switch (ser_subtype) {
    ...
    case GET_MAX_FILES:
      ...
      status = 0;
      memcpy(&packet->data[STATUS_BYTE], &status, sizeof(int8_t));
      memcpy(&packet->data[OUT_DATA_BYTE], &new_max_files,
             sizeof(new_max_files));

      set_packet_length(packet, sizeof(int8_t) + sizeof(new_max_files) +
                                    1);  // +1 for subservice
    ...
    case GET_HK:
      ...
      csp_buffer_free(packet);
      if (fetch_historic_hk_and_transmit(conn, limit, before_id, before_time) !=
          SUCCESS) {
        return SATR_ERROR;
      }
      break;
 ...
 * @details
 *      Accepts incoming housekeeping service packets and executes the
 * application
 * @param void* param
 * @return None
 */
