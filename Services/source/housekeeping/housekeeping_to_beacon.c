...
void update_beacon(All_systems_housekeeping* all_hk_data) {
  ...
  //TODO: Athena MCU temperature should not be stored as long data type since data size can change depending on the computer
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (athena_MCU_temp_long[i] > 127) {athena_MCU_temp_long[i] = 127;}
    if (athena_MCU_temp_long[i] < -128) {athena_MCU_temp_long[i] = -128;}
  ...
  memcpy(&(beacon_packet.pckts_unrecovered_by_FEC), &(all_hk_data->Athena_hk.pckts_uncovered_by_FEC), sizeof(uint16_t));
