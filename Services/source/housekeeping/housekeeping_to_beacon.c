...
void update_beacon(All_systems_housekeeping *all_hk_data) {
    ...
    beacon_packet.time = all_hk_data->hk_timeorder.UNIXtimestamp;

    /*-------EPS-------*/
    beacon_packet.eps_mode = all_hk_data->EPS_hk.battMode;
    beacon_packet.battery_voltage = all_hk_data->EPS_hk.vBatt;
    beacon_packet.battery_input_current = all_hk_data->EPS_hk.curBattIn;
    ...
    beacon_packet.output_states = all_hk_data->EPS_hk.outputStatus;
    ...
    beacon_packet.EPS_boot_count = all_hk_data->EPS_hk.bootCnt;
    ...
    beacon_packet.eps_last_reset_reason = EPS_last_reset_reason;

    /*-------Watchdog-------*/
    // TODO: More work to be done on watchdogs
    beacon_packet.gs_wdt = all_hk_data->EPS_hk.outputConverterState;
    beacon_packet.obc_wdt = all_hk_data->EPS_hk.vBatt;
    beacon_packet.gs_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
    beacon_packet.obc_wdt_expr = all_hk_data->EPS_hk.vBatt;

    /*-------Temperatures-------*/
    // Onboard battery pack temp location defined in ICD
    beacon_packet.temps[0] = all_hk_data->EPS_hk.temp[8];
    ...
    beacon_packet.temps[1] = max_MCU_temp;

    beacon_packet.temps[2] = all_hk_data->UHF_hk.temperature;
    ...
    beacon_packet.temps[4] = all_hk_data->hyperion_hk.Nadir_Temp1;
    beacon_packet.temps[5] = all_hk_data->hyperion_hk.Zenith_Temp1;
    beacon_packet.temps[6] = all_hk_data->hyperion_hk.Port_Temp1;
    beacon_packet.temps[7] = all_hk_data->hyperion_hk.Star_Temp1;
    ...
    beacon_packet.angular_rate = max_rate;

    adcs_state adcs_current_hk;
    ADCS_get_current_state(&adcs_current_hk);
    beacon_packet.adcs_control_mode = adcs_current_hk.att_ctrl_mode;
    ...
    beacon_packet.uhf_uptime = UHF_uptime;
    ...
    beacon_packet.obc_boot_count = all_hk_data->Athena_hk.boot_cnt;
    beacon_packet.obc_last_reset_reason = all_hk_data->Athena_hk.last_reset_reason;
    beacon_packet.obc_mode = all_hk_data->Athena_hk.OBC_mode;

    // Unit will be deca-seconds due to size restraint, seconds = OBC_uptime*10
    beacon_packet.obc_uptime = all_hk_data->Athena_hk.OBC_uptime;
    beacon_packet.solar_panel_supply_curr = all_hk_data->Athena_hk.solar_panel_supply_curr;
    beacon_packet.obc_software_ver = all_hk_data->Athena_hk.OBC_software_ver;
    beacon_packet.obc_cmds_received = all_hk_data->Athena_hk.cmds_received;
    beacon_packet.pckts_unrecovered_by_FEC = all_hk_data->Athena_hk.pckts_uncovered_by_FEC;

    /*-------Logged Items-------*/
    // TODO: write the function for logged items in beacon_task
    beacon_packet.obc_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
    beacon_packet.obc_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
    beacon_packet.obc_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
    beacon_packet.obc_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
    beacon_packet.obc_wdt_expr = all_hk_data->EPS_hk.outputConverterState;
