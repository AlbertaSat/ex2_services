...
void update_beacon(All_systems_housekeeping *all_hk_data) {
    ...
    if (all_hk_data->EPS_startup_hk.last_reset_reason_reg = 0x0C800000) {
        EPS_last_reset_reason = Power_on;
    }
    if (all_hk_data->EPS_startup_hk.last_reset_reason_reg = 0x24000000) {
        EPS_last_reset_reason = IWDG;
    }
    if (all_hk_data->EPS_startup_hk.last_reset_reason_reg = 0x24000000) {
        EPS_last_reset_reason = NRST;
    }
    if (all_hk_data->EPS_startup_hk.last_reset_reason_reg = 0x14000000) {
